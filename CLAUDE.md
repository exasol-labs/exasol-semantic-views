...
The installed system creates five managed schemas plus one schema per published
model:
...
- `SEMANTIC_SOURCE` — principal-scoped catalog reads: one thin view per
  `SYS_SEMANTIC` table the compiler reads, filtered to the models the caller is
  authorized for. **This is the surface callers are granted; `SYS_SEMANTIC` is
  not.** Generated by `package_lua_scripts.py` from the compiler's own reads, so
  a new catalog read either has a view or fails the build
...
(BUG-23). A virtual-schema table is bound against a local stand-in built from
its column metadata, never through the adapter (BUG-23b). Metric expressions,
filters and identity expressions are not probed, so invalid dialect syntax there can still pass
