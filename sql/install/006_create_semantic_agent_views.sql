...
SELECT
  ...
  TRUE AS SESSION_SETUP_REQUIRED,
  'EXECUTE SCRIPT SEMANTIC_ADMIN.ENABLE_SEMANTIC_SQL()' AS SESSION_SETUP_SQL,
  m.SURFACE_TYPE,
  vr.VALIDATION_RUN_ID AS LATEST_VALIDATION_RUN_ID,
  vr.STATUS AS VALIDATION_STATUS,
  vr.ERROR_COUNT AS VALIDATION_ERROR_COUNT,
  COALESCE(pc.PRECONDITION_COUNT, 0) AS VALIDATION_PRECONDITION_COUNT,
  vr.WARNING_COUNT AS VALIDATION_WARNING_COUNT,
  CASE
    WHEN m.STATUS <> 'PUBLISHED' THEN 'NOT_PUBLISHED'
    WHEN vr.VALIDATION_RUN_ID IS NULL THEN 'NEEDS_VALIDATION'
    WHEN vr.ERROR_COUNT > 0 THEN 'INVALID'
    WHEN COALESCE(pc.PRECONDITION_COUNT, 0) > 0 THEN 'PRECONDITION'
...
LEFT JOIN (
  ...
  ON pc.VALIDATION_RUN_ID = vr.VALIDATION_RUN_ID
...
SELECT
  ...
  -- Summarised for the root entity, which is the grain the object's metrics
  ...
  COALESCE(
    ...
    (
      ...
      WHERE afp.ENTITY_ID = so.ROOT_ENTITY_ID
        ...
        AND afp.FUSION_STRATEGY IN ('COALESCE', 'RECONCILE')
    ),
    'NONE'
  ) AS FUSION_STRATEGY
...
CREATE OR REPLACE VIEW SEMANTIC_AGENT.EXPRESSION_FUNCTIONS_FOR_AGENT AS
SELECT 'ABS' AS FUNCTION_NAME, 'NUMERIC' AS FUNCTION_CATEGORY
UNION ALL SELECT 'AVG', 'AGGREGATE'
UNION ALL SELECT 'CAST', 'CONVERSION'
UNION ALL SELECT 'CEIL', 'NUMERIC'
UNION ALL SELECT 'COALESCE', 'CONDITIONAL'
UNION ALL SELECT 'CONCAT', 'STRING'
UNION ALL SELECT 'COUNT', 'AGGREGATE'
UNION ALL SELECT 'DATE_TRUNC', 'DATE_TIME'
UNION ALL SELECT 'DAY', 'DATE_TIME'
UNION ALL SELECT 'EXTRACT', 'DATE_TIME'
UNION ALL SELECT 'FLOOR', 'NUMERIC'
UNION ALL SELECT 'LOWER', 'STRING'
UNION ALL SELECT 'LPAD', 'STRING'
UNION ALL SELECT 'LTRIM', 'STRING'
UNION ALL SELECT 'MAX', 'AGGREGATE'
UNION ALL SELECT 'MIN', 'AGGREGATE'
UNION ALL SELECT 'MONTH', 'DATE_TIME'
UNION ALL SELECT 'NULLIF', 'CONDITIONAL'
UNION ALL SELECT 'REPLACE', 'STRING'
UNION ALL SELECT 'ROUND', 'NUMERIC'
UNION ALL SELECT 'RTRIM', 'STRING'
UNION ALL SELECT 'SUBSTR', 'STRING'
UNION ALL SELECT 'SUM', 'AGGREGATE'
UNION ALL SELECT 'TO_CHAR', 'CONVERSION'
UNION ALL SELECT 'TO_DATE', 'CONVERSION'
UNION ALL SELECT 'TRIM', 'STRING'
UNION ALL SELECT 'TRUNC', 'DATE_TIME_OR_NUMERIC'
UNION ALL SELECT 'UPPER', 'STRING'
UNION ALL SELECT 'YEAR', 'DATE_TIME';
...
-- What fusion is in play, as data.
--
...
-- one -- and none of it was visible on the agent surface. An agent reading only
...
--
-- One row per fusion declaration, keyed by entity: representations and their
-- partition coverage, authority roles, attribute policies, and certified
-- identity mappings. FUSION_ASPECT says which kind of declaration a row is.
CREATE OR REPLACE VIEW SEMANTIC_AGENT.FUSION_FOR_AGENT AS
SELECT
  er.MODEL_ID,
  er.MODEL_NAME,
  er.VERSION_ID,
  er.ENTITY_ID,
  er.ENTITY_NAME,
  CASE WHEN er.COVERAGE_PREDICATE IS NULL THEN 'REPRESENTATION' ELSE 'PARTITION' END
    AS FUSION_ASPECT,
  er.REPRESENTATION_NAME,
  CAST(NULL AS VARCHAR(32)) AS ATTRIBUTE_TYPE,
  CAST(NULL AS VARCHAR(256)) AS ATTRIBUTE_NAME,
  CASE WHEN er.COVERAGE_PREDICATE IS NULL THEN er.REPRESENTATION_ROLE ELSE 'UNION' END
    AS STRATEGY,
  er.COVERAGE_PREDICATE,
  er.VALID_FROM,
  er.VALID_TO,
  er.SOURCE_SCHEMA || '.' || er.SOURCE_OBJECT AS SOURCE_OBJECT
FROM SEMANTIC_CATALOG.ENTITY_REPRESENTATIONS er
...
SELECT
  ...
  ra.ENTITY_ID,
  ra.ENTITY_NAME,
  'AUTHORITY' AS FUSION_ASPECT,
  ra.REPRESENTATION_NAME,
  NULL,
  NULL,
  ra.AUTHORITY_ROLE AS STRATEGY,
  NULL,
  NULL,
  NULL,
  NULL
FROM SEMANTIC_CATALOG.REPRESENTATION_AUTHORITIES ra
...
SELECT
  ...
  afp.ENTITY_ID,
  afp.ENTITY_NAME,
  'ATTRIBUTE_POLICY' AS FUSION_ASPECT,
  NULL,
  afp.ATTRIBUTE_TYPE,
  afp.ATTRIBUTE_NAME,
  afp.FUSION_STRATEGY AS STRATEGY,
  NULL,
  NULL,
  NULL,
  NULL
FROM SEMANTIC_CATALOG.ATTRIBUTE_FUSION_POLICIES afp
...
SELECT
  ...
  imr.ENTITY_ID,
  imr.ENTITY_NAME,
  'IDENTITY' AS FUSION_ASPECT,
  imr.REPRESENTATION_NAME,
  'IDENTITY',
  imr.IDENTITY_NAME,
  imr.CERTIFICATION_STATUS AS STRATEGY,
  NULL,
  NULL,
  NULL,
  imr.SOURCE_SCHEMA || '.' || imr.SOURCE_OBJECT AS SOURCE_OBJECT
FROM SEMANTIC_CATALOG.IDENTITY_MAPPING_RELATIONS imr
WHERE imr.STATUS = 'ACTIVE';
...
SELECT
  -(m.MODEL_ID * 10 + 1) AS INSTRUCTION_ID,
  m.MODEL_ID,
  m.MODEL_NAME,
  m.VERSION_ID,
  m.VERSION_NUMBER,
  'MODEL' AS SCOPE_TYPE,
  CAST(NULL AS DECIMAL(18,0)) AS SCOPE_ID,
  m.MODEL_NAME AS SCOPE_NAME,
  'SAFETY' AS INSTRUCTION_KIND,
  'Before using SEMANTIC_SQL against ' || m.PUBLISHED_SCHEMA
    || ', execute ' || m.SESSION_SETUP_SQL
    || ' in the same session. This sets SQL_PREPROCESSOR_SCRIPT to '
    || m.PREPROCESSOR_QUALIFIED_NAME
    || '; reapply it after reconnecting. STRUCTURED_REQUEST does not require session setup.'
    AS INSTRUCTION_TEXT,
  'PUBLIC' AS APPLIES_TO_ROLE,
  0 AS PRIORITY,
  'ACTIVE' AS STATUS,
  CAST(NULL AS TIMESTAMP) AS CREATED_AT,
  'SEMANTIC_ADMIN' AS CREATED_BY
FROM SEMANTIC_AGENT.MODELS_FOR_AGENT m
WHERE m.MODEL_STATUS = 'PUBLISHED'
UNION ALL
SELECT
  -(m.MODEL_ID * 10 + 2) AS INSTRUCTION_ID,
  m.MODEL_ID,
  m.MODEL_NAME,
  m.VERSION_ID,
  m.VERSION_NUMBER,
  ...
  CAST(NULL AS DECIMAL(18,0)) AS SCOPE_ID,
  m.MODEL_NAME AS SCOPE_NAME,
  'PRECONDITION' AS INSTRUCTION_KIND,
  'Query modes: STRUCTURED_REQUEST uses SEMANTIC_ADMIN.COMPILE_REQUEST_JSON; '
    || 'SEMANTIC_SQL requires SESSION_SETUP_SQL. Before VALIDATE_MODEL or '
    || 'federated multi-representation probes, set ALTER SESSION SET QUERY_TIMEOUT=60.'
    AS INSTRUCTION_TEXT,
  'PUBLIC' AS APPLIES_TO_ROLE,
  1 AS PRIORITY,
  'ACTIVE' AS STATUS,
...
SELECT
  ...
  END AS ENTRY_TEXT,
  QUERY_MODES,
  CAST(NULL AS VARCHAR(2000000)) AS REQUEST_JSON
FROM SEMANTIC_AGENT.FIELDS_FOR_AGENT
UNION ALL
SELECT
  MODEL_ID,
...
CREATE OR REPLACE VIEW SEMANTIC_AGENT.MODEL_EVOLUTION_REVIEW_QUEUE AS
SELECT
  e.SUGGESTION_ID,
  e.MODEL_ID,
  e.MODEL_NAME,
  e.PROPOSED_VERSION_ID,
  e.PROPOSED_VERSION_NUMBER,
  ...
  e.OBJECT_ID,
  e.OBJECT_NAME,
  e.PROPOSED_CHANGE_JSON,
  e.RATIONALE,
  e.REVIEW_STATUS,
  e.CREATED_AT,
  e.CREATED_BY
FROM SEMANTIC_CATALOG.MODEL_EVOLUTION_SUGGESTIONS e
JOIN SEMANTIC_AGENT.MODELS_FOR_AGENT visible
  ON visible.MODEL_ID = e.MODEL_ID
