...
def mark_repro_objects_deleted(con) -> None:
    statements = [
        ...
        # Bindings must be removed BEFORE their owning DIMENSION/FACT rows go
        # away, otherwise the name-based JOIN below cannot resolve them and
        # they become orphans that break subsequent PUBLISH_MODEL calls with
        # SEMANTIC_MODEL_039 "Binding references an unknown dimension or fact."
        "DELETE FROM SYS_SEMANTIC.ATTRIBUTE_BINDINGS WHERE ATTRIBUTE_TYPE = 'DIMENSION' AND ATTRIBUTE_ID IN (SELECT DIMENSION_ID FROM SYS_SEMANTIC.DIMENSIONS WHERE UPPER(DIMENSION_NAME) LIKE 'ZZ_REPRO_%')",
        "DELETE FROM SYS_SEMANTIC.ATTRIBUTE_BINDINGS WHERE ATTRIBUTE_TYPE = 'FACT' AND ATTRIBUTE_ID IN (SELECT FACT_ID FROM SYS_SEMANTIC.FACTS WHERE UPPER(FACT_NAME) LIKE 'ZZ_REPRO_%')",
